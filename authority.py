...
class Authority:
    ...
    def issue_voter_key(self):
        voter_key = generate_keypair(voting_modulus=True)
        voting_signature = self.rsa_pss.sign(voter_key)
        return voter_key, voting_signature

    def issue_participant_key(self):
        participant_key = generate_keypair(participant_modulus=True)
        participant_signature = self.rsa_pss.sign(participant_key)
